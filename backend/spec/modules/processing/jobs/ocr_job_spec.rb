  ...
  RSpec.describe OcrJob, type: :job do
    ...
    describe '#perform' do
      ...
      context 'when content_type is application/pdf and text > 100 chars' do
        ...
        it 'reports started, succeeds, and enqueues NLP' do
          pdf_text = 'A' * 150
          ...
          expect(Documents::ChangeDocumentStatus).to have_received(:call!).with(
            hash_including(document_id: document_hash[:document_id], status: :ocr_processing, action: :ocr_started)
          )
          expect(Documents::ChangeDocumentStatus).to have_received(:call!).with(
            hash_including(document_id: document_hash[:document_id], status: :ocr_succeeded, action: :ocr_succeeded)
          )
      ...
      context 'when content_type is application/pdf but direct text short' do
        ...
        it 'falls back to image OCR for each page' do
          ...
          expect(Documents::ChangeDocumentStatus).to have_received(:call!).with(
            hash_including(status: :ocr_processing, action: :ocr_started)
          )
          expect(Documents::ChangeDocumentStatus).to have_received(:call!).with(
            hash_including(status: :ocr_succeeded, action: :ocr_succeeded)
          )
          expect(NlpJob).to have_received(:perform_later).with(
      ...
      context 'when an exception is raised during OCR' do
        ...
        it 'logs error, reports failure, and re-raises' do
          ...
          expect(Documents::ChangeDocumentStatus).to have_received(:call!).with(
            hash_including(status: :ocr_retrying, action: :ocr_failed)
          )
        end
      end
    end
