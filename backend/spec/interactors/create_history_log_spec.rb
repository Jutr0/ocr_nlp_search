...
RSpec.describe CreateHistoryLog, type: :interactor do
  ...
  it 'succeeds' do
    context = described_class.call(document: pending_document, action: DocumentHistoryLog.actions[:created])
    expect(context).to be_success
  end

  context 'raises RecordInvalid' do
    it 'when action is nil' do
      context = described_class.call(document: pending_document, action: nil)
    ...
    it 'when document is nil' do
      context = described_class.call(document: nil, action: DocumentHistoryLog.actions[:created])
